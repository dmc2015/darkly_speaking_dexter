def parse_line(self, text: str, line_number: int) -> Optional[Dict]:
        ...
        self.logger.debug(f"[Line {line_number}] Current speaker at start: {self.current_speaker}")
        ...
        if text.startswith('[') and text.endswith(']'):
            ...
            for word in words:
                ...
                if normalized_word in self.name_normalizer.case_insensitive_mappings:
                    ...
                    self.logger.debug(f"[Line {line_number}] Found potential speaker: {speaker_name}")
                elif any(action.lower() in word.lower() for action in self.action_words):
                    context_words.append(word)
            ...
            if context_words:
                self.context_buffer.extend(context_words)
                self.logger.debug(f"[Line {line_number}] Updated context buffer: {self.context_buffer}")
            
            if speaker_name:
                previous_speaker = self.current_speaker
                self.current_speaker = self.name_normalizer.normalize(speaker_name)
                self.logger.debug(f"[Line {line_number}] Speaker change: {previous_speaker} -> {self.current_speaker}")
            
            self.logger.debug(f"[Line {line_number}] Current speaker after processing: {self.current_speaker}")
            return None
        #######
