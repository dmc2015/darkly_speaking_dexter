...
class DexterScraper:
    ...
    def get_episode_links(self) -> List[str]:
        ...
        try:
            ...
            episode_links = [urljoin(self.base_url, link['href']) for link in links if link.get('href')]
    ...
    def is_speaker_line(self, text: str) -> Tuple[Optional[str], str]:
        ...
        if match:
            ...
            if not any(word.lower() in speaker.lower() for word in ['music', 'rings', 'click', 'sound', 'phone']):
    ...
    def parse_episode(self, url: str) -> Optional[Dict]:
        ...
        try:
            ...
            content = soup.find('div', class_='content') or soup.find('div', class_='postbody')
            if not content:
      ...
      try:
          ...
          metadata = {
              ...
              'unique_speakers': len(set(
                  d['speaker'] for ep in self.episodes_data 
                  for d in ep['dialogue'] if 'speaker' in d
              ))
          }
