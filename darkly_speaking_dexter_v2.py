...
class DexterScraper:
        ...
        for element in content.children:
            if isinstance(element, NavigableString):
                ...
                if current_line:
                    ...
                    if sentence_buffer and not text.strip()[-1] in '.!?"\')}]' and not text.strip().endswith('...'):
                        ...
                        if sentence_buffer:
                            ...
                            if not text.strip()[-1] in '.!?"\')}]' and not text.strip().endswith('...'):
    ...
    def is_speaker_line(self, text: str) -> Tuple[Optional[str], str]:
        ...
        if match:
            ...
            # List of common transcript actions/sounds to filter out
            non_speaker_words = {
                'music', 'rings', 'click', 'sound', 'phone',
                'grunts', 'sighs', 'laughs', 'groans', 'coughs',
                'whimpers', 'screams', 'whistles', 'gasps', 'chuckles',
                'breathing', 'footsteps', 'silence', 'rustling',
                'door opens', 'door closes', 'knocking',
                'crying', 'sobbing', 'sniffling', 'snoring',
                'muttering', 'mumbling', 'whispering',
        ...
        try:
            ...
            return {
                ...
                'metadata': {
                    ...
                    'unique_speakers': len(set(d['speaker'] for d in dialogue if 'speaker' in d))
                }
            }
        except requests.RequestException as e:
